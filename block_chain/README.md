##EOS代码分析
本文主要针对EOS源码chain,chainbase部分代码分析，涉及的关键源码目录：
eos/libraries/chain
eos/libraries/chainbase
eos/libraries/fc
eos/plugins/producer_plugin
主要涉及区块的数据结构分析，区块生产机制分析，区块打包机制分析，transaction打包及执行机制分析，区块存储和数据库回滚机制的分析
当然其他部分源码也有涉猎，在此不做详细讨论。
EOS源码版本为1.2.1
开发环境：
...
关键术语：
	account:账户
	action:
	permission:加权的安全机制，用来确定一个action是否被正确授权
	transaction:
